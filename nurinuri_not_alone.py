...
if df_elder is not None and df_facility is not None:
    ...
    if elder_region_col_candidates:
        ...
        st.warning("독거노인 지역 컬럼을 자동으로 찾을 수 없습니다.")
    ...
    if target_col_candidates:
        ...
        st.warning("독거노인 인구 컬럼을 자동으로 찾을 수 없습니다.")
    ...
    facility_region = facility_region_col_candidates[0] if facility_region_col_candidates else st.selectbox("의료기관 지역 컬럼 선택", df_facility.columns)
    ...
    # 충청북도 누락 방지 — 유사 이름 일괄 수정
    df_elder["지역"] = df_elder["지역"].replace({
        "충청북": "충청북도", "충북": "충청북도", "충청북 도": "충청북도"
    })
    df_facility["지역"] = df_facility["지역"].replace({
        "충청북": "충청북도", "충북": "충청북도", "충청북 도": "충청북도"
    })
    ...
    if target_col is not None and target_col in df_elder.columns:
        ...
        # 이상치 제거 후 평균 계산 (충북 보정)
        q1, q3 = df["독거노인_1000명당_의료기관_수"].quantile([0.25, 0.75])
        iqr = q3 - q1
        filtered = df[(df["독거노인_1000명당_의료기관_수"] >= q1 - 1.5*iqr) & (df["독거노인_1000명당_의료기관_수"] <= q3 + 1.5*iqr)]
        mean_ratio = filtered["독거노인_1000명당_의료기관_수"].mean()
        ...
        fig = px.choropleth(
            ...
            title=f"시도별 독거노인 **1000명당** 의료기관 분포 (이상치 제외 전국 평균: {mean_ratio:.2f})",
        ...
        st.error("독거노인 인구 컬럼이 설정되지 않아 시각화를 진행할 수 없습니다.")
else:
